...
describe("Player", () => {
  ...
  it("should add a card", () => {
    ...
    mockPlayer.addCardsToHand(new Card(10, 0));
  ...
  describe("remove card from table", () => {
    ...
    it("Should remove a card from somewhere", () => {
      mockPlayer.removeCards(mockPlayer.hand[0]);

      expect(mockPlayer.hand).toHaveLength(3);
      expect(mockPlayer.table.flat()).toHaveLength(6);

      mockPlayer.removeCards(mockPlayer.table[0][0]);
      expect(mockPlayer.table[0].length).toBe(1);
      expect(mockPlayer.table.flat().length).toBe(5);
    });

    it.skip("Should throw because card is both in hand and on table", () => {
      mockPlayer.hand = [new Card(1, 1)];
      mockPlayer.table = [[new Card(1, 1)], [], []];

      expect(() => mockPlayer.removeCards(new Card(1, 1))).toThrow();
    });

    it.skip("Should throw because the card to be removed is not found", () => {
      expect(() => mockPlayer.removeCards(new Card(9, 1))).toThrow();
    });
  });
});
